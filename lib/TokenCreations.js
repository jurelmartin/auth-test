...
exports.createTokens = async (user, ATSecret, RTSecret ) => {
    ...
    const createToken = jwt.sign({ user:user.dataValues.id }, ATSecret, { expiresIn: '1m' });

    const createRefreshToken = jwt.sign({ user:user.dataValues.id }, RTSecret, { expiresIn: '7d' });
